...
export const onRequestPut: PagesFunction<Env> = async ({ env, params, request }) => {
  try {
    ...
    if (existing.slug === 'main-crate') {
      ...
      const heightTolerance = nullableInteger(body.heightToleranceMm, 'Height overflow tolerance', 0, 500)
      if (overflow === null) throw new HttpError(400, 'Overflow limit is required.')
      if (heightTolerance === null) throw new HttpError(400, 'Height overflow tolerance is required.')
